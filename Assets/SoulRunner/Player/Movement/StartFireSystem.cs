...
namespace SoulRunner.Player.Movement
{
  ...
  {
    private readonly EcsFilterInject<Inc<FireCommand, PlayerViewRef, Kelli>, Exc<HoldFireTimer>> _fire = default;
    ...
    public void Run(IEcsSystems systems)
    {
      foreach (int index in _fire.Value)
      {
        ...
        {
          _world.Add<Firing>(index)
            .Hand = commandHand;
          kelli.NextHand = GetNextHand(commandHand);

          _world.Add<HoldFireTimer>(index)
            .TimeLeft = _hero.Value.HoldFireTime;
